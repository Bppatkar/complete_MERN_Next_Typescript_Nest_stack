...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (!updateComment) throw new ApiError(500, "Failed to update comment");
...
const deleteComment = asyncHandler(async (req, res) => {
  ...
  if (!commentId || !isValidObjectId(commentId))
    throw new ApiError(400, "deleteComment :: Comment Id is not valid");
  ...
  if (req.user?._id.toString() !== comment.owner.toString())
    throw new ApiError(
      401,
      "deleteComment :: You do not have permission to perform this action"
    );
